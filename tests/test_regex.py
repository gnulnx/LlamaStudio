...
def process_message_content(content_str: str):
    ...
    content_str = re.sub(r'<\|im_start|>response:.*?(?:<\|im_end\|>|<\|im_end>|$)', '', content_str, flags=re.DOTALL)
    ...
    p1 = re.finditer(r'<\|tool_call>call:([\w\.:]+)([\(\{].*?[\)\}])<tool_call\|>', content_str, re.DOTALL)
    for m in p1:
    ...
    p3 = re.finditer(r'<(?:tool_code|tool_call|function)>\s*([\w\.:]+)([\(\{].*?[\)\}])\s*</(?:tool_code|tool_call|function)>', content_str, re.DOTALL)
    for m in p3:
        tool_matches.append((m.group(0), m.group(1), m.group(2)))
    ...
    content_str = re.sub(r'<\|channel>thought.*?(?:<channel\|>|<\|channel\|>|<channel>|$)', '', content_str, flags=re.DOTALL)
    content_str = re.sub(r'<think>.*?</think>', '', content_str, flags=re.DOTALL)
    
    print("Parsed Tool Calls:")
    print(synthesized_tool_calls)
    print("\nCleaned Content:")
    print(repr(content_str.strip()))
    print("=" * 60)
