...
def _server_error_details(status_code: int, response_body: str) -> dict:
    """Turn a llama-server error response into concise user-facing feedback."""
    message = response_body.strip() or f"llama-server returned HTTP {status_code}."
    ...
    hint = "Review the model server logs for more detail, then retry the message."
    ...
    if context_match:
        requested, available = (int(value) for value in context_match.groups())
        title = "Context window exceeded"
        hint = (
            f"This turn needs {requested:,} tokens, but the model is loaded with "
            f"{available:,}. Start a new chat, reduce the conversation or tool output, "
            "or reload the model with a larger context size."
        )
...
class ChatManager:
    ...
    def stream_chat(
        ...
        while iteration < max_iterations:
            ...
            for msg in conv.messages:
                # Local transport errors are useful conversation history for the user,
                # but they were not generated by the model and must not be sent back.
                if msg.error:
                    continue
                ...
                with httpx.Client(timeout=timeout) as client:
                    ...
                    with client.stream("POST", url, json=payload) as resp:
                        if resp.status_code != 200:
                            error = _server_error_details(
                                resp.status_code,
                                resp.read().decode(errors="replace"),
                            )
                            logger.warning("[chat] %s: %s", error["title"], error["message"])
                            conv.messages.append(Message(role="assistant", error=error))
                            self._save_to_disk()
